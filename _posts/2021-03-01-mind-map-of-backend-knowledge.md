...
理由并不是因为算法不重要，而是因为网络上已经有了各种总结，也有很多刷题和联系工具，想要提高必须一步一个脚印地刷题。另外，一般算法也只有字节和外企大厂比较爱问，就不在这里做太多总结了。

## 操作系统

### 为什么操作系统基础重要

操作系统其实是一门计算机的基础课，也是最应该学好的课，因为太多的东西都可以在操作系统中找到影子，很多系统的设计都可以借鉴操作系统中对应功能的设计。从这个角度来看，本质上软件开发就是不停地重复造定制化的轮子，或者说必须站在成熟系统的基础上，才有可能做出优雅的设计、开发出健壮的系统。

不过在平时的工作中，各种繁琐的日常会让人逐渐忘记很多重要的知识，温故而知新，每次再拿起这些内容总会有新的收获。可能是受了初中课本上的三个W和三个H的影响，对于一个组件和功能我总是要先了解它产生的原因，否则就不能很好地理解，因此后面大部分知识点都会先讲背景，再说它的原理。
...
物理地址：计算机系统的主存（包含内存、硬盘、其他IO等等）被组织成一个连续的长度为M的字节数组，每一个字节都有唯一的物理地址。
...
与物理内存类似，虚拟内存也可以被组织成一个连续的长度为N的字节数组。为了方便管理，系统将寻你内存分割成许多个大小为P的虚拟页，物理内存也分割成许多个大小为P的虚拟页，这样可以方便地管理虚拟内存，并将虚拟内存与物理内存进行映射。
